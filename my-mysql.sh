...
# 文件路径
sys_result="$dir/$current_datetime-mysql_sys.txt"
latency="$dir/$current_datetime-mysql_lan.txt"
vmstate="$dir/$current_datetime-mysql_vmstate.txt"
...
monitor_performance() {
    sleep 3
    /bin/python3 my-mysql.py --operation monitor --duration $monitor_duration >"mysql_latency_results.txt" &
    local pid_mysql=$!
    ...
    cat "mysql_latency_results.txt" >>$latency
