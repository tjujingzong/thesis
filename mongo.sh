...
# 文件路径
sys_result="$dir/$current_datetime-mongo_sys.txt"
latency="$dir/$current_datetime-mongo_lan.txt"
vmstate="$dir/$current_datetime-mongo_vmstate.txt"
...
monitor_performance() {
    sleep 3
    /bin/python3 mongo.py --operation monitor --duration $monitor_duration >"mongo_latency_results.txt" &
    local pid_mongo=$!
    ...
    cat "mongo_latency_results.txt" >>$latency
