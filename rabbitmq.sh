...
dir="results_rabbitmq"
mkdir -p $dir
current_datetime=$(date +%Y%m%d%H%M)

# 文件路径
sys_result="$dir/$current_datetime-rabbitmq_sys.txt"
latency="$dir/$current_datetime-rabbitmq_lan.txt"
vmstate="$dir/$current_datetime-rabbitmq_vmstate.txt"
...
monitor_performance() {
    sleep 3
    /bin/python3 rabbitmq.py --operation monitor --duration $monitor_duration >"rabbitmq_latency_results.txt" &
    local pid_rabbitmq=$!
    ...
    cat "rabbitmq_latency_results.txt" >>$latency
