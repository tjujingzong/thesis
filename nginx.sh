...
# 文件路径
sys_result="$dir/$current_datetime-nginx_sys.txt"
latency="$dir/$current_datetime-nginx_lan.txt"
vmstate="$dir/$current_datetime-nginx_vmstate.txt"
...
monitor_performance() {
    sleep 3
    /bin/python3 nginx.py >"nginx_latency_results.txt" &
    local pid_nginx=$!
    ...
    cat "nginx_latency_results.txt" >>$latency
