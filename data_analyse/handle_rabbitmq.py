...
# 打开TXT文件并读取内容
with open('../results_rabbitmq/202403211137-rabbitmq_lan.txt', 'r') as txt_file:
    lines = txt_file.readlines()

# 创建CSV文件并写入数据
with open('../data_analyse/data_result/202403211137-rabbitmq_lan.csv', 'w', newline='') as csv_file:
    csv_writer = csv.writer(csv_file)

    # 添加列名
    column_names = ['stress-vm', 'stress-cpu(%)', 'stress-io(%)', 'avg-latency(ms)']
...
lines = read_file('../results_rabbitmq/202403211137-rabbitmq_sys.txt')
content = ''
...
# 保存cpi 为csv文件
df.to_csv("./data_result/202403211137-rabbitmq_sys.csv", index=False)
...
lines = read_file('../results_rabbitmq/202403211137-rabbitmq_vmstate.txt')
...
# 将平均值写入文件
with open('../data_analyse/data_result/202403211137-rabbitmq_vmstate.csv', 'w') as f:
    f.write('\n'.join(averages))
...
df1 = pd.read_csv('../data_analyse/data_result/202403211137-rabbitmq_vmstate.csv', names=column_names_with_units)

# 保存
df1.to_csv('../data_analyse/data_result/202403211137-rabbitmq_vmstate.csv', index=False)

# 读取CSV文件
df1 = pd.read_csv('../data_analyse/data_result/202403211137-rabbitmq_lan.csv')
df2 = pd.read_csv('../data_analyse/data_result/202403211137-rabbitmq_sys.csv')
df3 = pd.read_csv('../data_analyse/data_result/202403211137-rabbitmq_vmstate.csv')
...
# 将合并后的数据保存到新的CSV文件
combined_df.to_csv('../data_analyse/data_result/202403211137-rabbitmq-all_data.csv', index=False)
